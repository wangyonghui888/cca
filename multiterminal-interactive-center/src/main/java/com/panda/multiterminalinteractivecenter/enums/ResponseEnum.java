...
public enum ResponseEnum implements BaseEnum {
    ...
    TOKEN_IS_EXPIRED("0400035", "TOKEN_IS_EXPIRED", "用户TOKEN已经过期", false),
    TOKEN_IS_REQUIRED("0400036", "TOKEN_IS_REQUIRED", "用户TOKEN必传", false),
    SYSTEM_ERROR("0400037", "SYSTEM_ERROR", "系统异常，请联系管理员！", false),
    DATA_NOT_EXIST("0400038", "DATA_NOT_EXIST", "记录不存在", false),
    NOT_ALLOW_UPDATE("0400039", "NOT_ALLOW_UPDATE", "活动入口全为关的情况下不允许更新", false),
    SERIAL_NUMBER_EXIST("0400039", "SERIAL_NUMBER_EXIST", "流水号已存在", false),
    ...
    MARCHANT_NAME_IS_EXIST("0410001", "MARCHANT_NAME_IS_EXIST", "商户名已存在", false),
    MARCHANT_CODE_IS_EXIST("0410002", "MARCHANT_CODE_IS_EXIST", "商户编码已存在", false),
    MARCHANT_ADMIN_NAME_IS_EXIST("0410003", "MARCHANT_ADMIN_NAME_IS_EXIST", "商户管理员名称已存在", false),


    MERCHANT_LEVEL_IS_EXIST("0410004", "MARCHANT_CODE_IS_EXIST", "商户等级已存在", false),
    MERCHANT_KEY_EXPIRE_ISSUE("0410005", "MERCHANT_KEY_EXPIRE_ISSUE", "商户key不得超过商户有效期", false),
    MERCHANT_CHILD_EXPIRE_PARENT("0410005", "MERCHANT_KEY_EXPIRE_ISSUE", "二级商户不得超过父级商户有效期", false),
    MERCHANT_PARENT_NOT_EXIST("0410006", "MERCHANT_PARENT_NOT_EXIST", "父级商户不存在!", false),
    MERCHANT_LEVEL_NOT_EXIST("0410007", "MERCHANT_LEVEL_NOT_EXIST", "商户等级不存在!", false),
    MERCHANT_AMOUNT_EXCEED_LIMIT("0410008", "MERCHANT_AMOUNT_EXCEED_LIMIT", "商户数量超限!", false),
    ...
    ResponseEnum(String id, String code, String label, boolean success) {
        this.id = id;
        this.code = code;
        this.label = label;
    ...
    public static ResponseEnum getErrorMsg(String msg) {
        if (StringUtils.isEmpty(msg)) {
            return ResponseEnum.INTERNAL_ERROR;
        }
        ResponseEnum[] resps = ResponseEnum.values();
        for (int i = 0; i < resps.length; i++) {
            ResponseEnum resp = resps[i];
