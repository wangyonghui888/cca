...
FROM (SELECT o1.id,
             ...
             o1.dangerous_id,
             o1.pre_order
      FROM tybss_new_bak.t_order_his o1
      where o1.modify_time >= :sql_last_value
        AND o1.modify_time <= UNIX_TIMESTAMP(now()) * 1000 - 1000 * 60 * 1
        AND o1.modify_time>=1667232000000
      ORDER BY o1.modify_time asc
      LIMIT 150000) o
         LEFT JOIN tybss_new_bak.t_settle_his s ON s.order_no = o.order_no and s.last_settle = 1 and s.out_come in (2, 3, 4, 5, 6) and
                                 s.bet_amount > 0
