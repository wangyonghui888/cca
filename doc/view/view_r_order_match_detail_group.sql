DROP VIEW IF EXISTS tybss_report.view_r_order_match_detail_group;
CREATE ALGORITHM = UNDEFINED DEFINER =`root`@`localhost` SQL SECURITY DEFINER VIEW `tybss_report`.`view_r_order_match_detail_group` AS
select uuid()                                                                        AS `id`,
       ...
       `tor`.`currency_code`                                                          AS `order_currency_code`
from (
      (
          (
              (
                  (
                      (
                          (
                              (
                                  (`tybss_merchant_common`.`t_order_detail` `tod` left join `tybss_merchant_common`.`s_match_info` `smi` on
                                      ((`smi`.`id` = `tod`.`match_id`)))
