...
FROM   (SELECT Sum(CASE
       ...
       LEFT JOIN (SELECT Sum(Cast(o.order_amount_total AS BIGINT)) AS bet_amount,
                         ...
                         Sum(CASE
                                 WHEN From_unixtime(Cast(Substring(first_bet_date, 0, 10) AS BIGINT), 'yyyyMMdd HH:mm:ss') <= Date_format('2020-03-06', 'yyyyMMdd 11:59:59')
                                      AND From_unixtime(Cast(Substring(first_bet_date, 0, 10) AS BIGINT), 'yyyyMMdd HH:mm:ss') >= Date_format(Date_add('2020-03-06', -1), 'yyyyMMdd 12:00:00') THEN 1
                                 ELSE 0
                  ...
                  FROM   base_temp.bss_order o
                         ...
                         LEFT JOIN base_temp.bss_settle s
                                ON s.order_no = o.order_no  and s.dt>='2020-03-06'
                  WHERE  o.dt = '2020-03-06'
       ...
       LEFT JOIN (SELECT Sum(Cast(d.bet_amount AS BIGINT))    AS live_bet_amount,
                  ...
                  FROM   dwm_report.order_match_detail d
                                ...
                                ON d.order_no = o.order_no  and o.dt >= '2020-03-06'
                         LEFT JOIN base_temp.bss_settle s
                                ON d.order_no = s.order_no and s.dt>='2020-03-06'
