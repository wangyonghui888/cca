...
BEGIN
          ...
          group by ru.merchant_code, ru.sport_id) a
             ...
             LEFT JOIN (SELECT m.merchant_code
                        ...
                        FROM tybss_merchant_common.t_merchant m
                                 left join tybss_merchant_common.t_merchant pm on pm.id = m.parent_id
    ...
    SELECT Concat(parent_code, sport_id, time)                                      id,
           ...
           floor(sum(order_valid_bet_money))  as                                    order_valid_bet_money,
           floor(sum(settle_valid_bet_money)) as                                    settle_valid_bet_money
