DROP PROCEDURE if EXISTS `p_r_merchant_finance_day_old`;
DELIMITER //

CREATE PROCEDURE `p_r_merchant_finance_day_old`(in execute_date varchar(100))
BEGIN
...
replace INTO tybss_report.r_merchant_finance_day
    (id, finance_day_id, finance_date, finance_time, merchant_id, merchant_code, merchant_name, agent_level, currency,
     ...
     order_valid_bet_money, settle_valid_bet_money, bet_promo_tickets, settle_promo_tickets, cashout_amount,
...
SELECT concat(execute_date, '-', m.merchant_code, '-', IFNULL(a.currency_code, '1'), '-', a.vip_level) as id,
       ...
       floor(IFNULL(b.settle_valid_bet_money, 0))                                                      as settle_valid_bet_money,
...
FROM (SELECT tm.id,
    ...
    when 3 and tm.agent_level = 0
          ...
          WHERE tm.agent_level in (0, 2)) as m
             LEFT JOIN (SELECT o.merchant_id,
                        ...
                        FROM tybss_merchant_common.t_order_old o
                                 LEFT JOIN tybss_merchant_common.t_settle_old s
             ...
             LEFT JOIN(SELECT u.merchant_code,
                       ...
                       FROM tybss_merchant_common.t_settle_old s
                                ...
                                left join tybss_merchant_common.t_order_old o on s.order_no = o.order_no
    ...
    (id, finance_day_id, finance_date, finance_time, merchant_id, merchant_code, merchant_name, agent_level, currency,
     ...
     vr_settle_original_platform_profit, order_valid_bet_money, settle_valid_bet_money, bet_promo_tickets,
...
SELECT concat(execute_date, '-', m.merchant_code, '-', IFNULL(bb.currency_code, '1'), '-', vip_level) as id,
       ...
       floor(IFNULL(bb.order_valid_bet_money, 0))                                                     as order_valid_bet_money,
       floor(IFNULL(bb.settle_valid_bet_money, 0))                                                    as settle_valid_bet_money,
...
SELECT concat(execute_date, '-', m.merchant_code, '-', IFNULL(a.currency_code, '1'), '-', a.vipLevel) as id,
       ...
       floor(IFNULL(a.settle_valid_bet_money, 0))                                                     as settle_valid_bet_money,
       floor(IFNULL(b.order_valid_bet_money, 0))                                                      as order_valid_bet_money,
    ...
    when 3 and tm.agent_level = 0
          ...
          WHERE tm.agent_level in (0, 2)) m
             LEFT JOIN (SELECT o.merchant_id,
                               ...
                               COUNT(distinct o.uid)                                              as cashout_users
                        FROM tybss_merchant_common.t_settle_old s
                                 LEFT JOIN tybss_merchant_common.t_order_old o on o.order_no = s.order_no
             ...
             LEFT JOIN (SELECT o.merchant_id,
                               ...
                               SUM(s.original_profit_amount) * -1                                           as original_platform_profit,
                               SUM(
                                       case when o.manager_code = 3 then o.original_order_amount_total end) as vr_original_order_amount_total,
                               SUM(case when o.manager_code = 3 then s.original_profit_amount end) *
                               -1                                                                           as vr_original_platform_profit,
                               SUM(CASE
                                       WHEN s.out_come in (3, 4, 5, 6) and
...
SELECT concat(execute_date, '-', m.merchant_code, '-', IFNULL(bb.currency_code, '1'), '-', vip_level) as id,
       ...
       floor(IFNULL(order_valid_bet_money, 0))                                                        as order_valid_bet_money,
       floor(IFNULL(settle_valid_bet_money, 0))                                                       as settle_valid_bet_money,
       IFNULL(bb.bet_promo_tickets, 0)                                                                as betPromoTickets,
       IFNULL(bb.settle_promo_tickets, 0)                                                             as settlePromoTickets,
