...
CREATE PROCEDURE `p_r_merchant_finance_day`(in execute_date varchar(100))
BEGIN
    ...
    (id, finance_day_id, finance_date, finance_time, merchant_id, merchant_code, merchant_name, agent_level, currency,
     ...
     settle_cashout_profit, cashout_tickets, cashout_users, settle_cashout_tickets, settle_cashout_users,original_cashout_profit_amount,original_cashout_amount,original_settle_cashout_money,original_settle_cashout_profit_amount)
    SELECT concat(execute_date, '-', m.merchant_code, '-', IFNULL(a.currency_code, '1'), '-', a.vip_level) as id,
           ...
           IFNULL(a.original_cashout_profit_amount,0) / 100                                                as original_cashout_profit_amount,
           IFNULL(a.original_cashout_amount,0) / 100                                                       as original_cashout_amount,
           IFNULL(b.original_settle_cashout_money,0) / 100                                                 as original_settle_cashout_money,
           IFNULL(b.original_settle_cashout_profit_amount,0) / 100                                         as original_settle_cashout_profit_amount
    FROM (SELECT tm.id,
                 tm.agent_level,
                 tm.merchant_code,
                 tm.merchant_name,
          ...
          WHERE tm.agent_level in (0, 2)) as m
             LEFT JOIN (SELECT o.merchant_id,
                               ...
                               SUM(case when o.manager_code = 3 then s.settle_amount end)               as vr_settle_amount,
                               ...
                               SUM(case when o.manager_code = 3 then s.profit_amount end) * -1          as vr_platform_profit,

                               SUM(o.original_order_amount_total)                                       as original_order_amount_total,
                               SUM(s.original_profit_amount) * -1                                       as original_platform_profit,
                               SUM(case when o.manager_code = 3 then o.original_order_amount_total end) as
                                                                                                           vr_original_order_amount_total,
                               SUM(case when o.manager_code = 3 then s.original_profit_amount end) *
                         ...
                         SUM(CASE
                               ...
                               count(CASE
                                         ...
                                         WHEN s.out_come in (3, 4, 5, 6) and
                                              ABS(o.order_amount_total) < ABS(s.profit_amount)
                                             THEN s.order_no END)                                       as betPromoTickets,

                               SUM(case when o.pre_bet_amount > 0 then s.profit_amount end)             as cashout_profit,
                               ...
                               SUM(case when o.original_pre_bet_amount > 0 then s.original_profit_amount end)             as original_cashout_profit_amount,
                        ...
                        GROUP BY o.merchant_id, o.currency_code, o.vip_level) a on a.merchant_id = m.id
             LEFT JOIN(SELECT u.merchant_code,
                              u.currency_code,
                              IFNULL(o.vip_level, 0)                                             as vip_level,
                              SUM(s.bet_amount)                                                  AS settle_order_amount_total,
                              ...
                              SUM(case when o.manager_code = 3 then s.bet_amount end)            AS vr_settle_order_amount_total,
                              SUM(case when o.manager_code = 3 then s.settle_amount end)         as vr_settle_settle_amount,
                              count(distinct (case when o.manager_code = 3 then s.order_no end)) as vr_settle_order_num,
                              Count(distinct (case when o.manager_code = 3 then s.uid end))      AS vr_settle_order_user_num,
                              Count(distinct (case when o.manager_code != 3 then s.uid end))     AS t_settle_order_user_num,
                              SUM(case when o.manager_code = 3 then s.profit_amount end) * -1    as vr_settle_platform_profit,

                              SUM(s.original_bet_amount)                                         as settle_original_order_amount_total,
                              SUM(s.original_profit_amount) * -1                                 as settle_original_platform_profit,
                              SUM(case when o.manager_code = 3 then s.original_bet_amount end)   as vr_settle_original_order_amount_total,
                              SUM(case when o.manager_code = 3 then s.original_profit_amount end) *
                         ...
                         SUM(CASE
                                      ...
                                      WHEN s.out_come in (3, 4, 5, 6) and
                                           ABS(o.original_order_amount_total) < ABS(s.original_profit_amount)
                                          THEN ABS(o.original_order_amount_total / 100) END)              as original_settle_valid_bet_acount,
                              count(CASE
                                        WHEN s.out_come in (3, 4, 5, 6) and
                                             ABS(o.order_amount_total) = ABS(s.profit_amount)
                                            THEN s.order_no
                                        ...
                                        WHEN s.out_come in (3, 4, 5, 6) and
                                             ABS(o.order_amount_total) < ABS(s.profit_amount)
                                            THEN s.order_no END)                                 as settlePromoTickets,

                              SUM(case when o.pre_bet_amount > 0 then s.profit_amount end)       as cashout_profit,
                              SUM(o.pre_bet_amount)                                              as cashout_amount,
                              COUNT(case when o.pre_bet_amount > 0 then o.order_no end)          as cashout_tickets,
                              COUNT(distinct o.uid)                                              as cashout_users,
                              SUM(o.original_pre_bet_amount)            as original_settle_cashout_money,
                              SUM(case when o.original_pre_bet_amount > 0 then s.original_profit_amount end)             as original_settle_cashout_profit_amount
                       FROM tybss_merchant_common.t_settle s
                                LEFT JOIN tybss_merchant_common.t_user u on u.uid = s.uid
                       ...
                       GROUP BY u.merchant_code, u.currency_code, o.vip_level) b
    ...
    SELECT concat(execute_date, '-', m.merchant_code, '-', IFNULL(bb.currency_code, '1'), '-', vip_level) as id,
           ...
           IFNULL(bb.original_cashout_profit_amount, 0)                                                   as original_cashout_profit_amount,
           IFNULL(bb.original_cashout_amount, 0)                                                          as original_cashout_amount,
           IFNULL(bb.original_settle_cashout_money, 0)                                                    as original_settle_cashout_money,
           IFNULL(bb.original_settle_cashout_profit_amount, 0)                                            as original_settle_cashout_profit_amount
    FROM (SELECT tm.id,
                 tm.agent_level,
                 tm.merchant_code,
                 tm.merchant_name,
    ...
    FROM (SELECT tm.id,
          ...
          WHERE tm.agent_level in (0, 2)) m
             LEFT JOIN (SELECT o.merchant_id,
                               ...
                               count(distinct (case when o.manager_code = 3 then s.order_no end)) as vr_settle_order_num,
                               Count(distinct (case when o.manager_code = 3 then s.uid end))      AS vr_settle_order_user_num,
                               Count(distinct (case when o.manager_code != 3 then s.uid end))     AS t_settle_order_user,
                               SUM(case when o.manager_code = 3 then s.profit_amount end) * -1    as vr_settle_platform_profit,
                        ...
                        where s.create_time >= startTimeUTCL
                          ...
                          and s.bet_amount > 0
                          and s.out_come in (2, 3, 4, 5, 6)
                        GROUP BY o.merchant_id, o.currency_code, vipLevel) a on a.merchant_id = m.id
             LEFT JOIN (SELECT o.merchant_id,
                               o.currency_code,
                               IFNULL(o.vip_level, 0)                                               as vipLevel,
                               SUM(o.order_amount_total)                                            AS order_amount_total,
                        ...
                        GROUP BY o.merchant_id, o.currency_code, vipLevel) b
                       on b.merchant_id = m.id and a.vipLevel = b.vipLevel and a.currency_code = b.currency_code
