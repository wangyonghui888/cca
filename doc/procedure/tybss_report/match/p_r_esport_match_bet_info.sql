DROP PROCEDURE IF EXISTS p_r_esport_match_bet_info;
DELIMITER //
CREATE PROCEDURE p_r_esport_match_bet_info(in execute_date varchar(100))
BEGIN
    /*声明日志信息*/
    DECLARE task_type INT(2) DEFAULT 212;
    DECLARE result_code CHAR(5) DEFAULT '0';
    DECLARE start_time VARCHAR(30) ;
    DECLARE end_time VARCHAR(30);
    DECLARE result INT(2) DEFAULT 1;
    DECLARE exce_msg VARCHAR(512) DEFAULT 'p_r_esport_match_bet_info成功';
    DECLARE msg TEXT;
...
/*异常处理*/
    DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
        GET DIAGNOSTICS CONDITION 1 result_code = RETURNED_SQLSTATE, msg = MESSAGE_TEXT;
				 SET start_time = get_cur_ymdhms();
        SET end_time = get_cur_ymdhms();
        SET exce_msg = CONCAT('错误码:', result_code, execute_date, 'p_r_esport_match_bet_info错误信息：', msg);
        SET result = 2;
...
from (select id,
      ...
      where mi.start_time >= startTimeUTCL
        AND mi.start_time <= endTimeUTCL
     ) a
         left join
     (SELECT d.match_id                                                                             matchId,
             ...
             COUNT(DISTINCT (CASE WHEN d.series_type = 1 and d.order_status in (0, 1) THEN d.play_id END))
                                                                                                    play_amount,
             d.tournament_name                                                                 tournamentName,
      ...
      FROM view_r_esport_match_detail d
...
SET end_time = get_cur_ymdhms();
    ...
    SET exce_msg = CONCAT(execute_date, 'p_r_esport_match_bet_info success!');
