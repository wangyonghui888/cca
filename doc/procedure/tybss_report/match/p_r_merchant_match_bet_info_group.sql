/*存储过程*/
DROP PROCEDURE IF EXISTS tybss_report.p_r_merchant_match_bet_info_group;
DELIMITER //
CREATE PROCEDURE tybss_report.p_r_merchant_match_bet_info_group(in execute_date varchar(100))
BEGIN

    /*声明日志信息*/
    DECLARE task_type INT(2) DEFAULT 44;
...
/*异常处理*/
    ...
    SELECT CONCAT(d.match_id, d.merchant_code,d.order_currency_code)                              id,
           ...
           SUM(CASE
                   ...
                   WHEN s.out_come in (3, 4, 5, 6) and
                        ABS(d.order_amount_total) < ABS(s.profit_amount)
                       THEN ABS(d.order_amount_total / 100) END)                    valid_bet_amount,
           ...
           SUM(CASE
                   ...
                   WHEN s.out_come in (3, 4, 5, 6) and
                        ABS(d.original_order_amount_total) < ABS(s.original_profit_amount)
                       THEN ABS(d.original_order_amount_total / 100) END)                               as original_order_valid_bet_acount,
