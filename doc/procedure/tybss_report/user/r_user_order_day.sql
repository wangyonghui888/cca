...
BEGIN
    ...
    SELECT Concat(w.uid, executetimeL)                AS id
         ...
         , TRUNCATE(IFNULL(c.settle_valid_bet_money, 0),2) as settle_valid_bet_money
         ...
         , IFNULL(c.original_settle_order_amount, 0)           as original_settle_order_amount
         , IFNULL(c.original_settle_profit, 0)                 as original_settle_profit
         , IFNULL(a.original_profit, 0)                        as original_profit
         , IFNULL(a.original_valid_bet_amount, 0)              as original_valid_bet_amount
    FROM (SELECT o.uid
          ...
          WHERE (s.create_time >= startTimeL AND s.create_time <= endTimeL)) w
             LEFT JOIN (SELECT o.uid
                            ...
                            THEN ABS(o.original_order_amount_total / 100) END)                 as  original_order_valid_bet_money
                                , SUM(s.original_profit_amount)                                AS original_profit
             ...
             LEFT JOIN (SELECT s.uid
                            ...
                            ,SUM(s.original_bet_amount)                                        AS original_settle_order_amount
