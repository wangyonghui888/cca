CREATE OR REPLACE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `tybss_report`.`view_r_user_full_info` AS
SELECT
    `tu`.`uid` AS `uid`,
...
FROM
    (
     `tybss_merchant_common`.`t_user` `tu`
        LEFT JOIN `tybss_merchant_common`.`t_merchant` `tm` ON ((
            `tu`.`merchant_code` = `tm`.`merchant_code`
        )))
