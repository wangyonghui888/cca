...
BEGIN
    ...
    UPDATE tybss_new.t_user tu inner join (
        SELECT o.uid              as uid,
               max(o.create_time) as last_bet_time
        FROM tybss_new.t_order o
        where o.create_time >= unix_timestamp(DATE_ADD(execute_date, INTERVAL -1 HOUR)) * 1000 - 1
        group by o.uid) uf on tu.uid = uf.uid
    set tu.last_bet_time = uf.last_bet_time;
