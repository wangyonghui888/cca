...
drop event if EXISTS p_user_order_all;
drop event if EXISTS p_user_order_all_ALLDATA;
CREATE EVENT e_p_user_order_all ON SCHEDULE EVERY 1800 SECOND DO CALL p_user_order_all(DATE_ADD(CURRENT_DATE,INTERVAL -1 DAY),2,NULL);

drop event if EXISTS e_p_user_order_all_ALLDATA;
CREATE EVENT e_p_user_order_all_ALLDATA ON SCHEDULE EVERY 12 hour DO CALL p_user_order_all(DATE_ADD(CURRENT_DATE,INTERVAL -1 DAY),1,NULL);
...
CALL  p_user_order_all('2020-05-29',1,NULL);