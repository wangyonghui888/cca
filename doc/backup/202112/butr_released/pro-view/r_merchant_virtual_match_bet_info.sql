DROP PROCEDURE IF EXISTS `p_r_merchant_virtual_match_bet_info`;

DELIMITER //
CREATE PROCEDURE `p_r_merchant_virtual_match_bet_info`(in execute_date varchar(100))
BEGIN

    /*声明日志信息*/
    DECLARE task_type INT(2) DEFAULT 41;
    DECLARE result_code CHAR(5) DEFAULT '0';
    DECLARE start_time VARCHAR(30) DEFAULT get_cur_ymdhms();
    DECLARE end_time VARCHAR(30);
    DECLARE result INT(2) DEFAULT 1;
    DECLARE exce_msg VARCHAR(512) DEFAULT 'p_r_merchant_virtual_match_bet_info成功';
    DECLARE msg TEXT;
...
/*异常处理*/
    DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
        ...
        SET exce_msg = CONCAT("错误码：", result_code, execute_date, " p_r_merchant_virtual_match_bet_info 错误信息：", msg);
        SET result = 2;
...
SET exce_msg = CONCAT(execute_date, " p_r_merchant_virtual_match_bet_info success!");
CALL tybss_report.p_add_task_event_log(task_type, start_time, end_time, result, exce_msg);
/*执行成功，添加日志*/
SET end_time = get_cur_ymdhms();
