DROP PROCEDURE IF EXISTS `p_r_merchant_esport_match_bet_info`;

DELIMITER //
CREATE PROCEDURE `p_r_merchant_esport_match_bet_info`(in execute_date varchar(100))
BEGIN

    /*声明日志信息*/
    DECLARE task_type INT(2) DEFAULT 48;
    DECLARE result_code CHAR(5) DEFAULT '0';
    DECLARE start_time VARCHAR(30) DEFAULT get_cur_ymdhms();
    DECLARE end_time VARCHAR(30);
    DECLARE result INT(2) DEFAULT 1;
    DECLARE exce_msg VARCHAR(512) DEFAULT 'p_r_merchant_esport_match_bet_info成功';
    DECLARE msg TEXT;
...
/*异常处理*/
    DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
        ...
        SET exce_msg = CONCAT("错误码：", result_code, execute_date, " p_r_merchant_esport_match_bet_info 错误信息：", msg);
        SET result = 2;
...
FROM tybss_report.view_r_esport_match_detail d
         LEFT JOIN tybss_report.view_r_settle_full_detail s ON d.order_no = s.order_no
         left join  (select id,
                            sport_id,
                            start_time begin_time,
                            status match_status,
                            (select zs from tybss_merchant_common.s_esport_language where name_code = home_name_code)       homeName,
                            (select zs from tybss_merchant_common.s_esport_language where name_code = away_name_code)       awayName,
                            tournament_id,

                            (select zs from tybss_merchant_common.s_esport_language where name_code = tournament_name_code) tourNamentName
                     from tybss_merchant_common.s_esport_matches mi
                     where mi.start_time >= (UNIX_TIMESTAMP(execute_date) * 1000 + (12 * 60 * 60 * 1000))
                       AND mi.start_time <= (UNIX_TIMESTAMP(DATE_ADD(execute_date, INTERVAL 1 DAY)) * 1000 + (12 * 60 * 60 * 1000 - 1))
...
SET end_time = get_cur_ymdhms();
    ...
    SET exce_msg = CONCAT(execute_date, " p_r_merchant_esport_match_bet_info success!");
