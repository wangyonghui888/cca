DROP VIEW IF EXISTS tybss_report.view_r_esport_match_detail;
CREATE ALGORITHM = UNDEFINED DEFINER =`root`@`localhost` SQL SECURITY DEFINER VIEW `tybss_report`.`view_r_esport_match_detail` AS
select uuid()                                                                        AS `id`,
       date_format(from_unixtime((`smi`.`start_time` / 1000)), '%Y-%m')              AS `dt`,
       `smi`.`id`                                                                    AS `match_id`,
       `smi`.`sport_id`                                                              AS `sport_id`,
       `smi`.`tournament_id`                                                         AS `tournament_id`,
       `smi`.`start_time`                                                            AS `begin_time`,
       `smi`.`match_team`                                                             AS `match_team`,
       `smi`.`score`                                                                 AS `score`,
       `smi`.`status`                                                          AS `match_status`,
       `tod`.`id`                                                                    AS `bet_id`,
...
from (
      (
              ...
              (
                  (
                      (
                          ...
                          (
                              (`tybss_merchant_common`.`t_order_detail` `tod` left join `tybss_merchant_common`.`s_esport_matches` `smi` on
                                  ((`smi`.`id` = `tod`.`match_id`)))
         ...
         left join `tybss_merchant_common`.`s_esport_language` `sl1` on ((`sl1`.`name_code` = `smi`.`tournament_name_code`)));