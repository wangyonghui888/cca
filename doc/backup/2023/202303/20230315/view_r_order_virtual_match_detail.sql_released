DROP VIEW IF EXISTS tybss_report.view_r_order_virtual_match_detail;
CREATE  VIEW `tybss_report`.`view_r_order_virtual_match_detail` AS
select uuid()                                                                        AS `id`,
       ...
       `smi`.`show_batch_no`                                                          AS `tournament_name`,
       case `smi`.`sport_id`
           when 1001 then concat( (select zs from tybss_merchant_common.s_virtual_language where name_code = `smi`.`home_name_code`) , ' v ', (select zs from tybss_merchant_common.s_virtual_language where name_code = `smi`.`away_name_code`  ))
           when 1004 then concat( (select zs from tybss_merchant_common.s_virtual_language where name_code = `smi`.`home_name_code`) , ' v ', (select zs from tybss_merchant_common.s_virtual_language where name_code = `smi`.`away_name_code`  ))
           else `smi`.`batch_no` end        `match_info`,
       `tod`.`id`                                                                    AS `bet_id`,
              ...
              (
                  (
                      (
                          (
                              (
                                  (`tybss_merchant_common`.`t_order_detail` `tod`  join `tybss_merchant_common`.`s_virtual_match_info` `smi` on
                                      ((`smi`.`id` = `tod`.`match_id`)))
