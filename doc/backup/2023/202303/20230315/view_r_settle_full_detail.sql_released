DROP VIEW IF EXISTS tybss_report.view_r_settle_full_detail;
CREATE VIEW tybss_report.`view_r_settle_full_detail` AS
select date_format(from_unixtime((`ts`.`settle_time` / 1000)), '%Y-%m') AS `dt`,
       ...
       `ts`.`original_settle_amount`                                     AS `original_settle_amount`,
       `ts`.`original_bet_amount`                                       AS `original_bet_amount`,
       `ts`.`original_profit_amount`                                    AS `original_profit_amount`,
...
from ((((select * from `tybss_merchant_common`.`t_settle` where last_settle = 1 and bet_amount > 0) `ts`
    left join `tybss_merchant_common`.`t_order` `tor`
    on ((`ts`.`order_no` = `tor`.`order_no`)))
    left join `tybss_merchant_common`.`t_user` `tu`
    on ((`tu`.`uid` = `tor`.`uid`)))
