...
@Order(0)
@Slf4j
public class OrderRedisUtilBean {

    @Resource(name = "orderTemplate")
...
//        this.jedisCluster().setex(key, expireTime, value);
    ...
    public Boolean setAndTimeOut(String key, Object value, long ttl, TimeUnit timeUnit) {
    ...
    public <T> List<T> getByBatch(List<String> key) {
    ...
    public <T> List<T> getPrefixValues(String prefix) {
     ...
     * @return
     */
    public <T> List<T> getList(String key, Class<T> entityType) {
        String value = (String) redisTemplate.opsForValue().get(key);
        if (StringUtils.isNotBlank(value)) {
            return JsonUtils.jsonToObjectList(value, entityType);
        }
     ...
     * @return
     */
    public <V> V hGet(Object key, Object field) {
    ...
    public <V> List<V> hMultiGetPipelined(List<String> keys, Collection<Object> fields) {
        return (List<V>) redisTemplate.executePipelined(new RedisCallback() {
            @Override
            public Object doInRedis(RedisConnection redisConnection) throws DataAccessException {
                RedisSerializer keySerializer = redisTemplate.getKeySerializer();
                ...
                int counter = 0;
                Object hashKey;
                for (Iterator var6 = fields.iterator(); var6.hasNext(); rawHashKeys[counter++] = keySerializer.serialize(hashKey)) {
                    hashKey = var6.next();
                }
                for (String key : keys) {
                    byte[] key1 = keySerializer.serialize(key);
                    redisConnection.hMGet(key1, rawHashKeys);
                }
                return null;
            }
        });
    ...
    public void hPutTimeOut(String key, Object hashKey, Object value, long timeout, TimeUnit timeUnit) {
        redisTemplate.opsForHash().put(key, hashKey, value);
        redisTemplate.expire(key, timeout, timeUnit);
    }


    public <K, V> void hPutAll(String key, Map<K, V> maps, TimeUnit timeUnit, long timeout) {
     ...
     * @return
     */
    public <T> Set<T> sMembers(Object key) {
     ...
     * @return
     */
    public <T> Set<T> zReverseRange(Object key, long start, long end) {
    ...
    public void sendMsg(String channel, Object msg, String logPrefile) {
        log.info("{} 消息发送：{}", logPrefile, JsonUtils.objectToJson(msg));
    ...
    public static String compress(String str) {
        try {
            ...
            log.error("redis.compress:" + str, e);
            return null;
        }
    }
    ...
    public static String unCompress(String str) {
        try {
            ...
            log.error("redis.uncompress:" + str, e);
            return null;
        }
    }
}
