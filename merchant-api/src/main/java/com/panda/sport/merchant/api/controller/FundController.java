...
public class FundController {
     ...
     * @date: 2020/8/23 11:22
     */
    @PostMapping("/safety/transfer")
    @Trace
    public APIResponse transferSafety(HttpServletRequest request, @RequestParam(value = "userName") String userName,
                                @RequestParam(value = "merchantCode") String merchantCode,
                                @RequestParam(value = "transferType") String transferType,
                                @RequestParam(value = "amount") String amount,
                                @RequestParam(value = "transferId") String transferId,
                                  @RequestParam(value = "amountType", required = false) String amountType,
                                @RequestParam(value = "timestamp") Long timestamp,
                                @RequestParam(value = "safetyType") String safetyType,
                                @RequestParam(value = "signature") String signature) {
        log.info("api/fund/transferSynchronize  username:" + userName + ",merchantCode:" + merchantCode + ",amountType=" + amountType +
                ",transferType:" + transferType + ",amount:" + amount + ",transferId:" + transferId + ",timestamp:" + timestamp+",safetyType:"+safetyType);
        long startL = System.currentTimeMillis();
        if (StringUtils.isAnyEmpty(merchantCode, signature, userName, transferId, amount,safetyType) || timestamp == null) {
            return APIResponse.returnFail(ApiResponseEnum.PARAMETER_INVALID);
        }
        try {
            distributedLockerUtil.lock(RedisConstants.PAY_FAMILY_KEY + merchantCode + userName);
         return transferRecordService.transferSafety(request, merchantCode, userName, transferType, amount, transferId, timestamp, amountType,safetyType, signature);
        } catch (Exception e) {
            log.error("FundController.transferSynchronize,exception:" + transferId, e);
            return APIResponse.returnFail(ApiResponseEnum.INTERNAL_ERROR);
        } finally {
            log.info("api/fund/transferSynchronize:" + transferId + (System.currentTimeMillis() - startL));
            distributedLockerUtil.unLock(RedisConstants.PAY_FAMILY_KEY + merchantCode + userName);
        }
    }
