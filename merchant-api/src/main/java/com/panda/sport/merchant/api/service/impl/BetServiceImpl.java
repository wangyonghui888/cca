...
public class BetServiceImpl extends AbstractMerchantService implements BetService {
     ...
     */
    @Override
    public APIResponse<Object> queryBetListV2(HttpServletRequest request, String userName, Long startTime, Long endTime, String merchantCode, Integer sportId,
                                            Long tournamentId, Integer settleStatus, Integer pageNum, Integer pageSize, Long timestamp, String signature, Integer orderBy, String language) {
        try {
            if (queryBetListSwitch.equals("off")) {
                return APIResponse.returnFail(ApiResponseEnum.RATE_LIMIT_ERROR);
            ...
            APIResponse errorResult = checkParam(request, timestamp, merchantCode, signStr, signature);
            if (errorResult != null && !errorResult.getStatus()) {
                log.error("queryBetListV2:" + errorResult);
                return errorResult;
            }
            Long start7 = DateUtils.getStartTime();
            if (start7 > startTime) {
            ...
            orderBy = 2;
            Long cur = System.currentTimeMillis();
            log.info(signature + ",countBetListV2:param=" + ((pageNum - 1) * pageSize) + "," + pageSize);
            String key = userName + startTime + endTime + merchantCode + sportId + tournamentId + settleStatus + orderBy;
            String betCount = redisTemp.get(key);
            log.info(key + ",redis.countBetListBackUpV2:" + betCount);
            int count;
            if (StringUtils.isNotEmpty(betCount) && !betCount.equals("null")) {
                count = Integer.parseInt(betCount);
            } else {
                count = orderMapper.countBetList(userName, startTime, endTime, merchantCode, sportId, tournamentId, settleStatus, orderBy);
                if (endTime <= cur) {
                    log.info("queryBetListV2.endTime:" + endTime + ",cur=" + cur);
                    redisTemp.setWithExpireTime(key, Integer.toString(count), 3600);
                }
            }
            Map<String, Object> result = new HashMap<>();
            ...
            if (count > 0) {
                List<BetApiVo> betList;
                if (StringUtils.isNotEmpty(language)) {
                    betList = orderMapper.queryBetListBackUpByLgV2(userName, startTime, endTime, merchantCode, sportId, tournamentId,
                            settleStatus, (pageNum - 1) * pageSize, pageSize, orderBy, language);
                } else {
                    betList = orderMapper.queryBetListV2(userName, startTime, endTime, merchantCode, sportId, tournamentId,
                            settleStatus, (pageNum - 1) * pageSize, pageSize, orderBy);
                }
                for (BetApiVo vo : betList) {
                    assemblyHandicap(vo);
            ...
            log.error("BetController.queryBetListV2,exception!", e);
            return APIResponse.returnFail(ApiResponseEnum.INTERNAL_ERROR);
        }
    }
    ...
    /**
     * @description:查询两个月内已结算注单
     * @Param: [startTime, endTime, pageNum, pageSize]
     * @return: com.panda.sport.merchant.common.vo.api.APIResponse<java.lang.Object>
     * @date: 2021/01/29 11:27
     */
    @Override
    public APIResponse<Object> queryAllBetList(Long startTime, Long endTime, Integer pageNum, Integer pageSize) {
        try {
            Long start60 = DateUtils.getStartTimeSixty();
            if (start60 > startTime) {
                startTime = start60;
            }
            ...
            int count = orderMapper.countSettleOrderListBackUp(null, startTime, endTime, null);
            Map<String, Object> result = new HashMap<>();
            result.put("totalCount", count);
            if (count > 0) {
                List<BetApiVo> betList = orderMapper.querySettleOrderListBackUp(null, startTime, endTime, (pageNum - 1) * pageSize, pageSize, null);
                for (BetApiVo vo : betList) {
                    assemblyHandicap(vo);
                }
                result.put("list", betList);
            ...
            log.error("BetController.queryAllBetList,exception!", e);
            return APIResponse.returnFail(ApiResponseEnum.INTERNAL_ERROR);
        }
    }
    ...
    /**
     * @description:查询两个月内注单 按更新时间
     * @Param: [startTime, endTime, pageNum, pageSize]
     * @return: com.panda.sport.merchant.common.vo.api.APIResponse<java.lang.Object>
     * @date: 2021/02/26 11:27
     */
    @Override
    public APIResponse<Object> queryAllBetListByUpdateTime(Long startTime, Long endTime, Integer pageNum, Integer pageSize) {
        try {
            Long start60 = DateUtils.getStartTimeSixty();
            if (start60 > startTime) {
                startTime = start60;
            ...
            int count = orderMapper.countlBetListByUpdateTime(null, startTime, endTime);
            Map<String, Object> result = new HashMap<>();
            result.put("totalCount", count);
            if (count > 0) {
                List<BetApiVo> betList = orderMapper.queryAllBetListByUpdateTime(null, startTime, endTime, (pageNum - 1) * pageSize, pageSize);
                for (BetApiVo vo : betList) {
                    assemblyHandicap(vo);
                }
                result.put("list", betList);
    ...
    @Override
    public APIResponse<Object> getMerchantReport(Integer startTime, Integer endTime, String dateType) {

        try {
            List<?> convertList = rpcClient.getMerchantReportApi(startTime, endTime, dateType);
            ObjectMapper mapper = new ObjectMapper();
            List<ReportApiVo> resultList = mapper.convertValue(convertList, new TypeReference<List<ReportApiVo>>() {
            });

            return APIResponse.returnSuccess(resultList);

        } catch (Exception e) {
            log.error("BetController.getMerchantReport,exception!", e);
            return APIResponse.returnFail(ApiResponseEnum.INTERNAL_ERROR);
        }
    }

    @Override
    public APIResponse<Object> getBetReport(Long startTime, Long endTime, Integer reportDate, String merchantCode) {
        try {
            ReportApiVo reportApiVo;
            if (reportDate != null) {
                Map convert = rpcClient.getBetReport(reportDate, merchantCode);
                log.info("getBetReport:result:" + convert);
                if (convert != null) {
                    ObjectMapper mapper = new ObjectMapper();
                    reportApiVo = mapper.convertValue(convert, new TypeReference<ReportApiVo>() {
                    });
                } else {
                    reportApiVo = null;
                }
            } else {
                reportApiVo = orderMapper.getMerchantStatistic(startTime, endTime, merchantCode);
            }
            return APIResponse.returnSuccess(reportApiVo);
        } catch (Exception e) {
            log.error("BetController.getBetReport,exception!", e);
            return APIResponse.returnFail(ApiResponseEnum.INTERNAL_ERROR);
        }
    }
    ...
    public APIResponse<Object> queryPreBetOrderList(HttpServletRequest request, Long startTime, Long endTime, String merchantCode,
        ...
        try {
            ...
            log.error("BetController.queryPreBetOrderList,exception!", e);
            return APIResponse.returnFail(ApiResponseEnum.INTERNAL_ERROR);
        }
    }
