...
public class RetryTransferServiceImpl extends AbstractMerchantService implements RetryTransferService {
    ...
    @Value("${no.second.settle.merchant}")
    private String noSecondSettleMerchant;
    ...
    public APIResponse addChangeRecordHistory(AccountChangeHistoryFindVO accountChangeHistoryFindVO, HttpServletRequest request) {
        ...
        try {
            ...
            if (BizTypeEnum.ADD_MONEY_MANUALLY.getCode() == bizType) {//增加
                vo.setChangeType(0);
                amount = amount + thisAmount;
            } else if (BizTypeEnum.MANUAL_DEBIT.getCode() == bizType) {//减少
                vo.setChangeType(1);
                amount = amount - thisAmount;
            }
    ...
    public String call(TransferRecordPO po, UserPO userPO, MerchantPO merchantPO, String userName, int callType) throws Exception {
        ...
        try {
            ...
            try {
                JSONArray obj = new JSONArray(orderJson);
                String orderNO = obj.getJSONObject(0).getString("orderNo");
                Integer settleType = settleMapper.querySettleTypeByOrderNo(orderNO);
                if (settleType != null && settleType > 3) {
                    if (transferType == 1) {
                        Integer secondSettleFlag = 2;
                        params.add("secondSettleFlag", secondSettleFlag);
                    }
                    if (noSecondSettleMerchant != null && noSecondSettleMerchant.contains(merchantPO.getMerchantCode())) {
                        transferRecordMapper.updateTransferRecord(transferId, STATUS_SUCCESS, "二次结算不支持商户");
                        transferRecordErrorMapper.updateTransferRecord(transferId, STATUS_SUCCESS, "二次结算不支持商户");
                        retryTransferResult(po, userPO.getUsername(), userName, STATUS_SUCCESS, "二次结算不支持商户!");
                        return "未知原因,商户加扣款失败!";
                    }
                }
            } catch (Exception e) {
                log.error(transferId + "JSONObject未知异常,商户扣款失败3!", e);
            }
            log.info(transferId + ",amountStr:" + amountStr);
            //免转商户验签兼容处理
            String key = "";
            String secondKey = "";
            if(StringUtils.isNotEmpty(merchantPO.getMerchantKey())){
                key = AESUtils.aesDecode(merchantPO.getMerchantKey());
            }
            if(StringUtils.isNotEmpty(merchantPO.getSecondMerchantKey())){
                secondKey = AESUtils.aesDecode(secondKey);
            }
            String singStr = userPO.getUsername() + "&" + po.getBizType() + "&" + merchantPO.getMerchantCode() +"&" + transferId + "&" + amountStr + "&" + transferType + "&" + now;
            if(StringUtils.isNotEmpty(key) && Constant.INT_1 == merchantPO.getKeyStatus()){
                params.add("signature",Md5Util.getMD5(singStr, key));
            }else if(StringUtils.isNotEmpty(secondKey) && Constant.INT_1 == merchantPO.getSecondStatus()){
                params.add("signature",Md5Util.getMD5(singStr, secondKey));
            }else if((StringUtils.isNotEmpty(key) && Constant.INT_1 == merchantPO.getKeyStatus())
                    && StringUtils.isNotEmpty(secondKey) && Constant.INT_1 == merchantPO.getSecondStatus()){
                params.add("signature",Md5Util.getMD5(singStr, key));
                //免转商户若启用双证书暂时默认传原始证书
//                params.add("signatureSecond",Md5Util.getMD5(singStr, secondKey));
            }
