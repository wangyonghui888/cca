...
public class RequestInterceptor {
    ...
    public Object Interceptor(ProceedingJoinPoint proceedingJoinPoint) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(merchantCode) && RequestLimitHand.requestLimit(request.getRequestURI(), merchantCode,userName))
            	return APIResponse.returnFail(ApiResponseEnum.RATE_LIMIT_ERROR);
            
            log.info("请求路径getRequestURL.rate.limit:" + (System.currentTimeMillis() - beginTime));
            // 一切正常的情况下，继续执行被拦截的方法
