...
import com.panda.sport.merchant.common.vo.merchant.TicketOrderSettleDetail;
 ...
 */
@Slf4j
@Service("abstractTicketOrderExportESService")
public abstract class AbstractTicketOrderExportESServiceImpl extends AbstractOrderFileExportService {
	@Autowired
	...
	public abstract List<TicketOrderSettle> processQuery(String filter, BetOrderVO betOrderVO);
	...
	public void execute(MerchantFile merchantFile) {
		...
		try {
			...
			List<TicketOrderSettle> orderSettleList;
			if (count < MerchantFileService.SPLIT_FILE_DATE_SIZE) {
				...
				try {
					if (count > pageSize) {
						...
						try {
							BlockingQueue<Future<List<TicketOrderSettle>>> queue = new LinkedBlockingQueue<Future<List<TicketOrderSettle>>>();
							for (int i = 0; i < thrednum; i++) {
								Future<List<TicketOrderSettle>> future = service.submit(read2List(i, filter, betOrderVO));
								queue.add(future);
							}
							int queueSize = queue.size();
							log.info("export  queueSize=" + queueSize);
							for (int i = 0; i < queueSize; i++) {
								List<TicketOrderSettle> list = queue.take().get();
				...
				for (int j = names.length; j > 0; j--) {
					...
					try {
						...
						try {
							BlockingQueue<Future<List<TicketOrderSettle>>> queue = new LinkedBlockingQueue<Future<List<TicketOrderSettle>>>();
							for (int i = 0; i < thrednum; i++) {
								Future<List<TicketOrderSettle>> future = service.submit(read2List(k, i, filter, betOrderVO));
								queue.add(future);
							}
							int queueSize = queue.size();
							log.info("export  queueSize=" + queueSize);
	...
	protected List<TicketOrderSettle> assembleBetOrderList(List<TicketOrderSettle> orderList, List<OrderSettle> userMerchantPOList){
	...
	protected byte[] exportOrderToCsv(List<TicketOrderSettle> resultList, String language,String merchantCode) throws Exception {
		List<LinkedHashMap<String, Object>> exportData =
				new ArrayList<>(CollectionUtils.isEmpty(resultList) ? 0 : resultList.size());
		Integer i = 0;

		for (TicketOrderSettle order : resultList) {
			try {
				...
				if (order.getOrderDetailList() != null) {
					if (StringUtils.isEmpty(merchantCode)) {
						for (TicketOrderSettleDetail detail:order.getOrderDetailList()) {
								...
								rowDataCopy.put("21", Objects.equals(order.getSeriesType(), 1) ?
										(language.equals(LanguageEnum.ZS.getCode()) ?
												("1".equals(detail.getMatchType())  ? "赛前" : ("2".equals(detail.getMatchType())  ? "滚球盘" : "冠军盘")) :
												("1".equals(detail.getMatchType())  ? "Prematch" : ("2".equals(detail.getMatchType())  ? "Live" : "Outright"))) :
										(language.equals(LanguageEnum.ZS.getCode()) ? "串关" : "Cross connection"));
								rowDataCopy.put("22", detail.getSportName());
								rowDataCopy.put("23", getObj(detail.getTournamentName()));
								rowDataCopy.put("24", detail.getMatchInfo());
								rowDataCopy.put("25", getObj(detail.getMatchId()));
								rowDataCopy.put("26", getObj(detail.getBeginTimeStr()));
								rowDataCopy.put("27", detail.getPlayName());
								rowDataCopy.put("28", detail.getPlayOptionName()+ "\t");
								rowDataCopy.put("29", getObj(detail.getMarketType()));
								rowDataCopy.put("30", getObj(detail.getMarketValue()));
								rowDataCopy.put("31", getObj(detail.getOddFinally()));
								rowDataCopy.put("32", getObj(detail.getIp()));
								rowDataCopy.put("33", detail.getDeviceImei());
						...
						if("trader".equals(merchantCode)){
							String matchType = order.getOrderDetailList().get(0).getMatchType();
							rowData.put("21", Objects.equals(order.getSeriesType(), 1) ?
									(language.equals(LanguageEnum.ZS.getCode()) ?
											("1".equals(matchType)  ? "赛前" : ("2".equals(matchType)  ? "滚球盘" : "冠军盘")) :
											("1".equals(matchType)  ? "Prematch" : ("2".equals(matchType)  ? "Live" : "Outright"))) :
									(language.equals(LanguageEnum.ZS.getCode()) ? "串关" :"Cross connection" ));
...
//1:H5，2：PC,3:Android,4:IOS
	...
	private Callable<List<TicketOrderSettle>> read2List(final int i, final String filter, final BetOrderVO betOrderVO) {
		long starTime = System.currentTimeMillis();
		Callable<List<TicketOrderSettle>> callable = new Callable<List<TicketOrderSettle>>() {
			@Override
			public List<TicketOrderSettle> call() throws Exception {
				List<TicketOrderSettle> list = new ArrayList<>();
				...
				for (int j = 0; j < 10; j++) {
					...
					try {
						List<TicketOrderSettle> listResult = processQuery(filter, parms);
	...
	private Callable<List<TicketOrderSettle>> read2List(final int j, final int i, final String filter, final BetOrderVO betOrderVO) {
		long starTime = System.currentTimeMillis();
		Callable<List<TicketOrderSettle>> callable = new Callable<List<TicketOrderSettle>>() {
			@Override
			public List<TicketOrderSettle> call() throws Exception {
