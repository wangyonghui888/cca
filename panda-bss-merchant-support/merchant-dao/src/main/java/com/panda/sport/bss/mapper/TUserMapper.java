...
public interface TUserMapper {
    ...
    UserPO getUserByUserName(@Param(value = "merchantCode") String merchantCode, @Param(value = "userName") String userName);

    UserPO getUserInfo(@Param(value = "uid") Long uid);

    @Select("select u.uid userId,u.special_betting_limit_type,u.special_betting_limit_time,u.special_betting_limit_remark from t_user u where u.uid=#{uid}")
    ...
    void batchUpdateLastLogin(@Param(value = "messages") List<Message> messages);
    ...
    List<String> queryUserIdListByUserName(@Param(value = "userName") String userName);

    @Select("select u.id as id  from t_user_level_relation u  where u.uid = #{uid}")
    UserLevelRelationVO getUserLevelRelationById(@Param(value = "uid") Long uid);

    void updateAllChildrenMerchantUser(@Param(value = "merchantCodeList") List<String> merchantCodeList, @Param(value = "disabled") int disabled);

    List<String> getAllUserName( @Param(value = "pageNum")long pageNum,  @Param(value = "pageCount")long pageCount);

    Long queryUserIdByName(@Param(value = "userName") String userName);
}
