...
public class RateLimiter {
     ...
     * 分布式同步锁
     */
    private DistributedLockerUtil syncLock;

    public RateLimiter(String key, Double permitsPerSecond, Integer maxBurstSeconds, DistributedLockerUtil syncLock) {
        this.key = key;
    ...
    private RedisPermits putDefaultPermits() {
        this.lock();
        try {
            Object obj = redisTemp.getObject(key, RedisPermits.class);
            if (null == obj) {
                RedisPermits permits = new RedisPermits(permitsPerSecond, maxBurstSeconds);
                redisTemp.setWithExpireTime(key, JSON.toJSONString(permits), (int) permits.expires());
                return permits;
            } else {
